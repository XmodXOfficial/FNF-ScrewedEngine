...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(ClientPrefs.showHealth) {
			healthTxt = new FlxText(botplayTxt.x,
				!ClientPrefs.downScroll ? botplayTxt.y + 100 : botplayTxt.y - 100,
				botplayTxt.fieldWidth,
			...
			healthTxt.borderSize = botplayTxt.borderSize;
	...
	override public function update(elapsed:Float)
	{
		...
		{
			var ret:Dynamic = callOnLuas('onChartEditor', []);
			if (ret != FunkinLua.Function_Stop)
			{
				switch (SONG.event7)
				{
		...
		{
			var ret:Dynamic = callOnLuas('onReset', []);
			if (ret != FunkinLua.Function_Stop)
			{
				health = 0;
				trace("RESET = True");
		...
		if (unspawnNotes[0] != null)
		{
			...
			if (unspawnNotes[0].multScroll < 1)
				time /= unspawnNotes[0].multScroll;
			...
			if (startedCountdown)
			{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					...
					if (strumScroll) // Downscroll
					{
						...
						daNote.distance = (0.45 * (Conductor.songPosition - daNote.strumTime) * songSpeed * daNote.multSpeed * daNote.multScroll);
					}
					else // Upscroll
					{
						// daNote.y = (strumY - 0.45 * (Conductor.songPosition - daNote.strumTime) * songSpeed);
	...
	public function endSong():Void
	{
		...
		else
		{
			var achieve:String = checkForAchievement([
				...
				'two_keys', 'toastie', 'debugger', 'bro_what'
			]);
